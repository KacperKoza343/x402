...
describe.each(HEDERA_KEY_TYPES)("$label client keys", ({ generate }) => {
...
describe.each(HEDERA_KEY_TYPES)("$label fee payer keys", ({ generate }) => {
  describe("createHederaSignAndSubmitTransaction", () => {
    ...
    async function buildTransferBase64(asset: string): Promise<string> {
      ...
      if (asset === "0.0.0") {
        ...
        tx.addTokenTransfer(tokenId, AccountId.fromString("0.0.9001"), -amount);
        tx.addTokenTransfer(tokenId, AccountId.fromString("0.0.7001"), amount);
      }
    ...
    it("rejects payloads that are not TransferTransactions", async () => {
      ...
      tx.setTransactionId(TransactionId.generate(AccountId.fromString(feePayerAccount)));
      tx.setSubmitKey(feePayerKey.publicKey);
      await tx.freezeWith(Client.forTestnet());
